...
/* -----------------------------------------------------------------------------------------* 
   Inputs:
   1. parentFolder: Location where the folder will get saved.
   2. folderName: Name of folder to be created.
...
%macro _cf_main_execution_code(parentFolder, folderName);
...
%if &_cf_run_trigger. = 1 %then %do;
   ...
   %_cf_main_execution_code(&parentFolder., &folderName.);
